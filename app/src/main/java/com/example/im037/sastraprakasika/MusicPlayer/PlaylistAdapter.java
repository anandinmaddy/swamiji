...
//        typeface = FontManager.getInstance(context).getTypeFace(FontManager.FONT_MATERIAL);
    ...
    public void onBindViewHolder(PlaylistItemHolder holder, int position) {
        ...
        Picasso.get().load(getImageUrl(currentItem.getYoutubeId())).into(holder.thumbnail);
...
//                    mCommonActivity.showAutoPlayItemPopUp(view,adapter.playlistItems.get(pos).videoId,adapter.playlistItems.get(pos).youtubeId,adapter.playlistItems.get(pos).title,adapter.playlistItems.get(pos).getUploader());
//
//                }
//            });
